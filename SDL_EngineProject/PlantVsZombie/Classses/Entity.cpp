...
void Entity::bindSprite(Sprite*sprite)
{
	...
	//Rect rect = m_pSprite->getSpriteFrameRect();
	//Size size = rect.size;

	this->setContentSize(size);
	m_pSprite->setPosition(size.width/2,size.height/2);
	//m_pSprite->setPosition(size.width/2 - rect.origin.x,size.height/2 - rect.origin.y);
	this->addChild(m_pSprite);
}
...
Sprite* Entity::bindSpriteWithSpriteFrame(SpriteFrame*spriteFrame)
{
	if(spriteFrame != nullptr)
	{
		...
		//重新设置content size 和 位置
		//sprite->getSpriteFrameRect();
