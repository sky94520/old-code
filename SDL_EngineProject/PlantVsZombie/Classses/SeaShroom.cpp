...
SeaShroom*SeaShroom::create(const string& plantName)
{
	auto plant = new SeaShroom();
...
bool SeaShroom::init(const string& plantName)
{
	//设置正常的动画
...
void SeaShroom::updateAtkState(float dt)
{
...
void SeaShroom::changeState(State state)
{
	if (m_state == state)
		return;
