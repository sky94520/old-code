...
TextureManager::TextureManager()
{
    if(IMG_Init(IMG_INIT_PNG) == 0)
        ...
        std::cerr<<"png initlized succeed!"<<std::endl;
}
TextureManager::~TextureManager()
{
    IMG_Quit();
...
bool TextureManager::load(std::string fileName, std::string id)
{
    ...
    {
        std::cout<<"Texture has existed!:"<<id<<"\n";
        return false;
    }
...
void TextureManager::draw(std::string id, int x, int y, int width, int height,SDL_RendererFlip flip)
{
    ...
    {
        std::cerr<<"Error:TextureManager draw() error: "<<id<<"not exist"<<std::endl;
    }
...
void TextureManager::drawFrame(std::string id, int x, int y, int width, int height, int currentRow,int currentFrame,double angle,int alpha, SDL_RendererFlip flip)
{
    ...
    {
        std::cerr<<"Error:TextureManager drawFrame() error: "<<id<<"not exist"<<std::endl;
    }
    SDL_Rect srcRect;
    SDL_Rect destRect;
