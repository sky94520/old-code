...
void PhysicalEngine::update(float time)
{
	...
	for(b2Body*b = m_pWorld->GetBodyList();b;b = b->GetNext())
	{
		...
		if(b->GetUserData())
		{
			...
			float32 b2Angle = - CC_DEGREES_TO_RADIANS(entity->getRotationX());
			//对鱼叉特殊处理
			if(b->GetFixtureList()->GetFilterData().categoryBits == 0x04)
			{
				Point pos = DynamicData::getInstance()->getOffsetRefraction()/PTM_RATIO;
				b2Position += b2Vec2(pos.x,pos.y);
			}
